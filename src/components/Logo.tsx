...
const Logo = ({ size = 'medium', withText = true }: LogoProps) => {
  ...
  return (
    <Link to="/" className="flex items-center gap-2">
      <div className={`relative ${container} flex items-center justify-center rounded-lg bg-gradient-to-r from-primary to-accent shadow-md`}>
        <Users className={`${iconMain} text-white absolute transform`} />
        <Heart className={`${iconAccent} text-white absolute top-1/2 left-1/2 transform -translate-x-1/2 -translate-y-1/2`} />
      </div>
