      ...
      <div className="container mx-auto px-4">
        <div className="text-center max-w-3xl mx-auto mb-12">
          ...
          <p className="text-gray-600 text-lg">
            نتعاون مع أفضل الجمعيات المتخصصة والمرخصة لضمان تقديم خدمات عالية الجودة لمستخدمي المنصة.
          </p>
        </div>
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-8">
          {associationsData.map((association) => (
            <div key={association.id} className="bg-white rounded-xl overflow-hidden shadow-lg transition-transform hover:transform hover:-translate-y-2 duration-300">
                  ...
                  <div className="flex items-center text-amber-500 mr-2">
                    {Array(5).fill(0).map((_, i) => (
                      <Star 
                        key={i} 
                        size={16} 
                        fill={i < Math.floor(association.rating) ? "currentColor" : "none"} 
                        className={i < Math.floor(association.rating) ? "text-amber-500" : "text-gray-300"}
                      />
                    ))}
                  </div>
                  <span className="text-sm text-gray-600">{association.rating}/5</span>
                </div>
